...
/***************USING roundTimestamp($time, $prec) INSTEAD*******/
/***********to round time punches versus rounding total hours***************/
...
function roundTimestamp($time, $prec='0') {
  switch($prec) {
	case '0' :
	  $round = 1;
	  break;
  ...
  $min_rnd = round($min/$round,0)*$round;

  $time_rnd =  mktime(str_pad($hour, 2, '0', STR_PAD_LEFT), $min_rnd, '00', $month, $day, $year);
  return $time_rnd;
}
