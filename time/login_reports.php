...
if (isset($_POST['login_userid']) && (isset($_POST['login_password']))) {
    ...
    $query = "select jobname, employee_passwd, reports from ".$db_prefix."jobs
              where jobname = '".$login_userid."'";
    $result = mysqli_query($db, $query);

    while ($row=mysqli_fetch_array($result)) {

        $reports_username = "".$row['jobname']."";
...
if (isset($_SESSION['valid_reports_user'])) {
    ...
    echo "  <tr class=right_main_text><td align=left>Jobname:</td><td align=right><input type='text' name='login_userid'></td></tr>\n";
