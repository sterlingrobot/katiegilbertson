...
if ($display_weather == "yes") {
    ...
    echo "        <div class='panel-heading'><b>$city</b><small class='text-info pull-right'>Last Updated: $time</small></div>\n";
	echo "		  <table class='table table-responsive'>\n";
    echo "        <tr><td>Currently: $temp&#176;</td><td align=left valign=middle class=misc_items>Feels Like: $feelslike&#176;</td>";
    if ($humidity == 'None') {
        echo "        <td>Humidity: $humidity</td></tr>\n";
    } else {
        echo "        <td>Humidity: $humidity%</td></tr>\n";
    }
    echo "        <tr><td>Skies: $cloud_cover</td><td>Wind: $wind_dir $wind$mph</td><td>Visibility: $visibility miles</td></tr>\n";
	echo "		  </table>\n";
}
////////////////////////////////////////////
...
function fetch_metar($station, $new) {
  ...
  if (is_array($file)) {
    ...
    $date_unixtime = mktime($date_parts[3], $date_parts[4],
  ...
  $date = date('Y/m/d H:i', $date_unixtime);
    ...
    if ($new) {
      ...
      $query = "INSERT INTO ".$db_prefix."metars SET station = '$station', " .
        "metar = '$metar', timestamp = '$date_unixtime'";
    } else {
