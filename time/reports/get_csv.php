...
for ($x=0;$x<$jobs_cnt;$x++) {
    ...
    if (($jobs_jobname[$x] == $fullname) || ($fullname == "All")) {
        ...
        while ($row=mysqli_fetch_array($result)) {
            ...
            if (strtolower($user_or_display) == "display") {
              $displayname = stripslashes($jobs_displayname[$x]);
            } else {
              $jobname = stripslashes($jobs_jobname[$x]);
            }
            $inout = "".$row["inout"]."";
            if (!empty($tmp_display_ip)) {
...
for ($x=0;$x<$jobs_cnt;$x++) {
          ...
          if ($tmp_info_date == $info_date[$y]) {
              if (empty($punchlist_in_or_out[$y])) {
                  ...
                  if ($y == $info_cnt - 1) {
                      ...
                      if ($tmp_show_details == "1") {
                          ...
                          if (strtolower($user_or_display) == "display") {
                              $string .= "$jobs_displayname[$x], $info_date[$y], $hours, ,\n";
                          } else {
                              $string .= "$jobs_jobname[$x], $info_date[$y], $hours, ,\n";
                          }
                      }
                      $secs = 0;
                      $punch_cnt = 0;
              ...
              if (isset($tmp_info_date)) {
                  ...
                  if ($tmp_show_details == "1") {
                      ...
                      if (strtolower($user_or_display) == "display") {
                          $string .= "$jobs_displayname[$x], $info_date[$yy], $hours, ,\n";
                      } else {
                          $string .= "$jobs_jobname[$x], $info_date[$yy], $hours, ,\n";
                      }
                  }
              $secs = 0;
              unset($in_time);
    ...
    if ($tmp_show_details == "1") {
        if (strtolower($user_or_display) == "display") {
            if (!empty($tmp_display_ip)) {
                $string .= "$jobs_displayname[$x], , , , , , , $my_total_hours,\n";
            } else {
                $string .= "$jobs_displayname[$x], , , , , , $my_total_hours,\n";
            }
        } else {
            if (!empty($tmp_display_ip)) {
                $string .= "$jobs_jobname[$x], , , , , , , $my_total_hours, \n";
            } else {
                $string .= "$jobs_jobname[$x], , , , , , $my_total_hours\n";
            }
        }
