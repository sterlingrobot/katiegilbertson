...
if($use_reports_password == "yes") {
  ...
  if(!isset($_SESSION['valid_reports_user'])) {
	...
	echo "  <tr><td>PHP Timeclock Reports</td></tr>\n";
	echo "  <tr>\n";
	echo "    <td>\n";
	echo "      <table class='table'>\n";
	echo "        <tr><td align=center>You are not presently logged in, or do not have permission to view this page.</td></tr>\n";
	echo "        <tr=><td align=center>Click <a class=admin_headings href='../login_reports.php'><u>here</u></a> to login.</td></tr>\n";
...
if($request == 'GET') {
  ...
  echo "            <table class='table'>\n";
  echo "              <tr>\n";
  echo "                <th colspan=3><img src='../images/icons/report.png' />&nbsp;&nbsp;&nbsp;Daily
                    Time Report</th></tr>\n";
  echo "              <tr><td></td></tr>\n";
  echo "              <input type='hidden' name='date_format' value='$js_datefmt'>\n";
  if($username_dropdown_only == "yes") {

	$query = "select * from " . $db_prefix . "jobs order by jobname asc";
...
// begin post error checking //
  ...
  if($fullname != "All") {
	$query = "select * from " . $db_prefix . "jobs where jobname = '" . $fullname . "'";
	$result = mysqli_query($db, $query);
	  ...
	  while($row = mysqli_fetch_array($result)) {
		...
		if(strtolower($user_or_display) == "display") {
		  echo stripslashes("              <tr class=display_row><td nowrap width=20% bgcolor='$row_color' style='padding-left:10px;
                      padding-right:10px;'>$jobs_displayname[$x]</td>\n");
		} else {
		  echo stripslashes("              <tr class=display_row><td nowrap width=20% bgcolor='$row_color' style='padding-left:10px;
                      padding-right:10px;'>$jobs_jobname[$x]</td>\n");
		}
